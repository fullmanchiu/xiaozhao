...
public class CalendarReminderUtils {
    ...
    public static void addCalendarEvent(Context context, String title, String description, long reminderTime, long endTime, int previousDate) {
        ...
        if (calId < 0) { //获取账户id失败直接返回，添加日历事件失败
            Toast.makeText(context, "添加日历提醒失败，请检查本机系统日历是否存在账户", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (newEvent == null) { //添加日历事件失败直接返回
            Toast.makeText(context, "添加日历提醒失败", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        Toast.makeText(context, "添加日历提醒成功", Toast.LENGTH_SHORT).show();
    }
